...
interval_number::operator*(const interval_number &b) const {
  ...
  case 10: // ++ * ++: <a0*b0, a1*b1>
    return interval_number(
        _mm_mul_pd(interval, _mm_xor_pd(b.interval, sign_low_mask())));
  }
...
inline interval_number interval_number::operator*(const double b) const {
  if (b >= 0)
    ...
    return interval_number(
        _mm_mul_pd(_mm_shuffle_pd(interval, interval, 1), _mm_set1_pd(-b)));
}
...
inline interval_number interval_number::operator/(const double b) const {
  if (b >= 0)
    ...
    return interval_number(
        _mm_div_pd(_mm_shuffle_pd(interval, interval, 1), _mm_set1_pd(-b)));
}
...
inline bigrational bigrational::operator+(const bigrational &r) const {
  if (sign == 0)
    ...
    // right_num, den3, -1); else if (sign > 0 && r.sign < 0) { 	if (left_num >=
    //right_num) return bigrational(left_num - right_num, den3, 1); 	else return
    //bigrational(right_num - left_num, den3, -1);
    // }
